...
func (m *MetaRecordIdx) Init() error {
	...
	err = cassUtils.EnsureTableExists(m.session.CurrentSession(), m.cfg.createKeyspace, m.cfg.keyspace, m.cfg.schemaFile, "schema_meta_record_table", m.cfg.metaRecordTable)
	if err != nil {
		return err
	}
	err = cassUtils.EnsureTableExists(m.session.CurrentSession(), m.cfg.createKeyspace, m.cfg.keyspace, m.cfg.schemaFile, "schema_meta_record_batch_table", m.cfg.metaRecordBatchTable)
	if err != nil {
		return err
	}
